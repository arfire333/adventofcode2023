...
int main(int argc, char* argv[]) {
  string filename = aoc::dataFilename("2023day09", "user.input");
  ...
  for (auto& history : data) {
    ...
    row--;
    while (row > 0) {
      history[row][S - row] = history[row][0] - history[row + 1][S - row - 1];
      row--;
    }
    cout << history[0][0] - history[1][S - 1] << "\n";
    sum += (history[0][0] - history[1][S - 1]);
  }
  cout << " sum=" << sum << "\n";
